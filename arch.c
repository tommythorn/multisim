...
isa_disass(FILE *out, const arch_t *arch, isa_decoded_t dec, isa_result_t res)
{
    ...
    default:
        ...
        if (dec.dest_msr != ISA_NO_REG)
            fprintf(out, " $%04x <- 0x%08x", dec.dest_msr, (uint32_t)res.msr_result);
        break;
    }
